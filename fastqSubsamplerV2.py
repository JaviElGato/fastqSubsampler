...
./fastqSubsampler --fastq=toy.fastq --prop=80 --outFastq=toy.out.fastq
"""
...
def main():
	...
	lineIndices = np.arange(numberOfitems, dtype=int)
	...
	for itera in range(1,2):
		...
		# itemsToExtract = random.sample(range(0,int(numberOfitems)),numberItemsToExtract)
		itemsToExtract = np.random.choice(lineIndices, size=numberItemsToExtract, replace=False)
